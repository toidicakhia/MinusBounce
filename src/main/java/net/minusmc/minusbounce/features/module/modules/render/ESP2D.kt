...
class ESP2D : Module() {
    ...
    private val absorption = BoolValue("Render-Absorption", true) {
        healthBar.get() && hpBarMode.get().equals("line", ignoreCase = true)
    }
    ...
    private val tagsBGValue = BoolValue("Tags-Background", true) { tagsValue.get() }
    ...
    private val vector: FloatBuffer
    ...
    init {
        ...
        backgroundColor = Color(0, 0, 0, 120).rgb
                ...
                if (position != null) {
                    ...
                    if (living) {
                        ...
                        if (health) {
                            ...
                            if (healthNumber.get() && (!hoverValue.get() || entity === mc.thePlayer || isHovering(
                                ...
                                posX - 4.0 - Fonts.minecraftFont.getStringWidth(
                                    if (hpMode.get()
                                            .equals("health", ignoreCase = true)
                                    ) healthDisplay else healthPercent
                                ) * fontScaleValue.get(),
                                endPosY - textWidth - Fonts.minecraftFont.FONT_HEIGHT / 2f * fontScaleValue.get(),
                    ...
                    if (armor) {
                        if (living) {
                            ...
                            if (itemStack.isItemStackDamageable) {
                                ...
                                if (armorNumber.get() && (!hoverValue.get() || entity === mc.thePlayer || isHovering(
                                        posX,
                                        endPosX,
                                        posY,
                                        endPosY,
                                    ...
                                    endPosX + 4.0,
                                    endPosY - durabilityWidth - Fonts.minecraftFont.FONT_HEIGHT / 2f * fontScaleValue.get(),
                                    fontScaleValue.get().toDouble(),
                                    -1
                                )
                                RenderUtils.newDrawRect(
                    ...
                    if (living && tagsValue.get()) {
                        ...
                        if (tagsBGValue.get()) RenderUtils.newDrawRect(
                            posX + (endPosX - posX) / 2f - (Fonts.minecraftFont.getStringWidth(entName) / 2f + 2f) * fontScaleValue.get(),
                            posY - 1f - (Fonts.minecraftFont.FONT_HEIGHT + 2f) * fontScaleValue.get(),
                            posX + (endPosX - posX) / 2f + (Fonts.minecraftFont.getStringWidth(entName) / 2f + 2f) * fontScaleValue.get(),
                        ...
                        drawScaledCenteredString(
                            ...
                            posY - 1f - Fonts.minecraftFont.FONT_HEIGHT * fontScaleValue.get(),
                            fontScaleValue.get().toDouble(),
                            -1
                        )
                    }
                    if (itemTagsValue.get()) {
                        if (living) {
                            ...
                            if (entityLivingBase.heldItem != null && entityLivingBase.heldItem.item != null) {
                                ...
                                if (tagsBGValue.get()) RenderUtils.newDrawRect(
                                    posX + (endPosX - posX) / 2f - (Fonts.minecraftFont.getStringWidth(itemName) / 2f + 2f) * fontScaleValue.get(),
                                    endPosY + 1f - 2f * fontScaleValue.get(),
                                    posX + (endPosX - posX) / 2f + (Fonts.minecraftFont.getStringWidth(itemName) / 2f + 2f) * fontScaleValue.get(),
                                    endPosY + 1f + (Fonts.minecraftFont.FONT_HEIGHT + 2f) * fontScaleValue.get(),
                                    -0x60000000
                                )
                                drawScaledCenteredString(
                                    itemName,
    ...
    private fun collectEntities() {
        ...
        while (i < playerEntitiesSize) {
            ...
            if (EntityUtils.isSelected(
                ...
                collectedEntities.add(entity as EntityLivingBase)
            }
            ++i
        }
    }
