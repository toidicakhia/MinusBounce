...
class FollowTargetHud : Module() {
    ...
    private fun getPlayerName(entity: EntityLivingBase): String {
        ...
        if (MinusBounce.fileManager.friendsConfig.isFriend(entity.name)) {
            pre = "$pre§b[Friend] "
        }
        ...
        if (!AntiBot.isBot(entity) && !teams.isInYourTeam(entity)) {
            pre = if (MinusBounce.fileManager.friendsConfig.isFriend(entity.name)) {
                "§b[Friend] §c"
            } else {
                "§c"
            }
    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String) {
        ...
        } else if (MinusBounce.combatManager.target == null) {
            targetTicks--
            if (targetTicks <= -1) {
                targetTicks = 0
