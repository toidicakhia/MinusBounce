...
import net.minusmc.minusbounce.utils.render.ColorUtils.stripColor
...
class TTFFontRenderer @JvmOverloads constructor(val font: Font, characterCount: Int = 256, fractionalMetrics: Boolean = true) {
    ...
    private fun setup(characterData: Array<CharacterData?>, type: Int): Array<CharacterData?> {
        ...
        for (index in characterData.indices) {
            ...
            characterData[index] =
                CharacterData(character, characterImage.width.toFloat(), characterImage.height.toFloat(), textureId)
        }
        ...
        for (i in 0 until length) {
            ...
            if (character == COLOR_INVOKER && i < length) {
                ...
                if (index < 16) {
                    ...
                    // Sets the character data to the regular type.
                    characterData = regularData
                    val shadowMul = if (shadow) 0.25 else 1.0
    ...
    fun getWidth(text: String): Float {
        ...
        for (i in 0 until length) {
            ...
            if (character == COLOR_INVOKER && i < length) {
                ...
                when (index) {
                    17 // Sets the character data to the bold type.
                    -> characterData = boldData
                    20 // Sets the character data to the italics type.
                    -> characterData = italicsData
                    21 // Sets the character data to the regular type.
                    -> characterData = regularData
                }
            } else {
                // Continues to not crash!
    ...
    fun getHeight(text: String): Float {
        ...
        for (i in 0 until length) {
            ...
            if (character == COLOR_INVOKER && i < length) {
                ...
                height = max(height.toDouble(), charData!!.height.toDouble()).toFloat()
            }
        }
