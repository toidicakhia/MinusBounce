...
class NullStyle : DropDownClickGui("Null") {
    ...
    override fun drawPanel(mouseX: Int, mouseY: Int, panel: Panel?) {
        RenderUtils.drawGradientSideways(panel!!.x.toFloat() - 3, panel.y.toFloat(), panel.x.toFloat() + panel.width + 3, panel.y.toFloat() + 19, ColorUtils.modifyAlpha(accentColor, 120).rgb, ColorUtils.modifyAlpha(accentColor!!.darker().darker(), 120).rgb)
        GlStateManager.resetColor()
        if (panel.getFade() > 0)
            RenderUtils.drawRect(panel.x.toFloat(),panel.y.toFloat() + 19,panel.x.toFloat() + panel.width,(panel.y + 19 + panel.getFade()).toFloat(),Int.MIN_VALUE)
        GlStateManager.resetColor()
        val textWidth = Fonts.font35.getStringWidth("§f" + StringUtils.stripControlCodes(panel.name)).toFloat()
        Fonts.font35.drawString("§f" + panel.name, (panel.x - (textWidth - 100.0f) / 2f).toInt(),panel.y + 7,Int.MAX_VALUE)
    }

    override fun drawDescription(mouseX: Int, mouseY: Int, text: String?) {
        val textWidth = Fonts.font35.getStringWidth(text!!)
        RenderUtils.drawRect((mouseX + 9).toFloat(), mouseY.toFloat(), (mouseX + textWidth + 14).toFloat(), (mouseY + Fonts.font35.FONT_HEIGHT + 3).toFloat(), ColorUtils.modifyAlpha(accentColor, 120).rgb)
        GlStateManager.resetColor()
    ...
    override fun drawValues(moduleElement: ModuleElement, mouseX: Int, mouseY: Int) {
        ...
        if (moduleValues.isNotEmpty()) {
            Fonts.font35.drawString("+", moduleElement.x + moduleElement.width - 10, moduleElement.y + moduleElement.height / 2, Color.WHITE.rgb)
    ...
    override fun drawListValue(value: ListValue, moduleElement: ModuleElement, mouseX: Int, mouseY: Int) {
        ...
        RenderUtils.drawRect((moduleElement.x + moduleElement.width + 4).toFloat(), (yPos + 2).toFloat(), moduleElement.x + moduleElement.width + moduleElement.settingsWidth, (yPos + 14).toFloat(), Int.MIN_VALUE)
        GlStateManager.resetColor()
        Fonts.font35.drawString("§c$text", moduleElement.x + moduleElement.width + 6, yPos + 4, 0xfffff)
