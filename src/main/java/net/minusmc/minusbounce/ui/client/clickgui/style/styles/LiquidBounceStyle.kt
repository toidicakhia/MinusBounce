...
class LiquidBounceStyle : Style() {
    override fun drawPanel(mouseX: Int, mouseY: Int, panel: Panel?) {
        RenderUtils.drawBorderedRect(
            ...
            panel.y.toFloat() + 19 + panel.getFade(),
            1f,
        ...
        if (panel.scrollbar && panel.getFade() > 0) {
            RenderUtils.drawRect(
                ...
                (panel.y + 21).toFloat(),
                panel.x - 0.5f,
                (panel.y + 16 + panel.getFade()).toFloat(),
                Int.MAX_VALUE
            )
            RenderUtils.drawRect(
                (panel.x - 2).toFloat(),
                panel.y + 30 + (panel.getFade() - 24f) / (panel.elements.size - MinusBounce.moduleManager.getModule(
                    ClickGUI::class.java
                )!!.maxElementsValue.get()) * panel.dragged - 10.0f,
                panel.x.toFloat(),
                panel.y + 40 + (panel.getFade() - 24.0f) / (panel.elements.size - MinusBounce.moduleManager.getModule(
                    ClickGUI::class.java
