...
class WhiteStyle : DropDownClickGui("White") {
    override fun drawPanel(mouseX: Int, mouseY: Int, panel: Panel?) {
        RenderUtils.drawBorderedRect(panel!!.x.toFloat(), panel.y.toFloat() - 3, panel.x.toFloat() + panel.width, panel.y.toFloat() + 17, 3f, Color(217, 217, 217).rgb, Color(217, 217, 217).rgb)
        if (panel.getFade() > 0) {
            RenderUtils.drawBorderedRect(panel.x.toFloat(), panel.y.toFloat() + 17, panel.x.toFloat() + panel.width, (panel.y + 19 + panel.getFade()).toFloat(), 3f, Color(217, 217, 217).rgb, Color(217, 217, 217).rgb)
            RenderUtils.drawBorderedRect(panel.x.toFloat(), (panel.y + 17 + panel.getFade()).toFloat(), panel.x.toFloat() + panel.width, (panel.y + 19 + panel.getFade() + 5).toFloat(), 3f, Color(217, 217, 217).rgb, Color(217, 217, 217).rgb)
        }
        GlStateManager.resetColor()
        val textWidth = Fonts.font35.getStringWidth("§f" + StringUtils.stripControlCodes(panel.name)).toFloat()
        Fonts.font35.drawString(panel.name, (panel.x - (textWidth - 100.0f) / 2f).toInt(), panel.y + 7 - 3, Color.BLACK.rgb)
    }

    override fun drawDescription(mouseX: Int, mouseY: Int, text: String?) {
        val textWidth = Fonts.font35.getStringWidth(text!!)
        RenderUtils.drawBorderedRect((mouseX + 9).toFloat(), mouseY.toFloat(), (mouseX + textWidth + 14).toFloat(), (mouseY + Fonts.font35.FONT_HEIGHT + 3).toFloat(), 3f, Color(217, 217, 217).rgb, Color(217, 217, 217).rgb)
        GlStateManager.resetColor()
        Fonts.font35.drawString(text, mouseX + 12, mouseY + Fonts.font35.FONT_HEIGHT / 2, Color.BLACK.rgb)
    }

    override fun drawButtonElement(mouseX: Int, mouseY: Int, buttonElement: ButtonElement?) {
        Gui.drawRect(buttonElement!!.x - 1, buttonElement.y - 1, buttonElement.x + buttonElement.width + 1, buttonElement.y + buttonElement.height + 1, ColorUtils.hoverColor(if (buttonElement.color != Int.MAX_VALUE) Color(14, 159, 255) else Color(255, 255, 255), buttonElement.hoverTime).rgb)
        GlStateManager.resetColor()
        Fonts.font35.drawString(buttonElement.displayName, buttonElement.x + 5, buttonElement.y + 7, Color.BLACK.rgb)
    }

    override fun drawModuleElement(mouseX: Int, mouseY: Int, moduleElement: ModuleElement?) {
        Gui.drawRect(moduleElement!!.x - 1, moduleElement.y - 1, moduleElement.x + moduleElement.width + 1, moduleElement.y + moduleElement.height + 1, ColorUtils.hoverColor(Color(255, 255, 255), moduleElement.hoverTime).rgb)
    ...
    override fun drawValues(moduleElement: ModuleElement, mouseX: Int, mouseY: Int) {
        ...
        if (moduleValues.isNotEmpty()) {
            ...
            if (moduleElement.isShowSettings) {
                if (moduleElement.settingsWidth > 0f && yPos > moduleElement.y + 6) RenderUtils.drawBorderedRect(
    ...
    override fun drawListValue(value: ListValue, moduleElement: ModuleElement, mouseX: Int, mouseY: Int) {
        ...
        Fonts.font35.drawString(text, moduleElement.x + moduleElement.width + 6, yPos + 2, -0x1000000)
    ...
    override fun drawFloatValue(value: FloatValue, moduleElement: ModuleElement, mouseX: Int, mouseY: Int) {
        val text = value.name + ": " + round(value.get()) + value.suffix
        val textWidth = Fonts.font35.getStringWidth(text).toFloat()
        if (moduleElement.settingsWidth < textWidth + 8) moduleElement.settingsWidth = textWidth + 8
        val valueOfSlide = drawSlider(value.get(), value.minimum, value.maximum, false, moduleElement.x + moduleElement.width + 8, yPos + 14, moduleElement.settingsWidth.toInt() - 12, mouseX, mouseY, Color(7, 152, 252))
        if (valueOfSlide != value.get()) value.set(valueOfSlide)
        Fonts.font35.drawString(text, moduleElement.x + moduleElement.width + 6, yPos + 3, -0x1000000)
        yPos += 19
    }

    override fun drawIntegerValue(value: IntegerValue, moduleElement: ModuleElement, mouseX: Int, mouseY: Int) {
        val text = value.name + ": " + if (value is BlockValue) getBlockName(value.get()) + " (" + value.get() + ")" else value.get().toString() + value.suffix
        val textWidth = Fonts.font35.getStringWidth(text).toFloat()
        if (moduleElement.settingsWidth < textWidth + 8) moduleElement.settingsWidth = textWidth + 8
        val valueOfSlide = drawSlider(value.get().toFloat(), value.minimum.toFloat(), value.maximum.toFloat(), true, moduleElement.x + moduleElement.width + 8, yPos + 14, moduleElement.settingsWidth.toInt() - 12, mouseX, mouseY, Color(7, 152, 252))
    ...
    override fun drawFontValue(value: FontValue, moduleElement: ModuleElement, mouseX: Int, mouseY: Int) {
        ...
        Fonts.font35.drawString(displayString, moduleElement.x + moduleElement.width + 6, yPos + 2, Color.BLACK.rgb)
    ...
    override fun drawTextValue(value: TextValue, moduleElement: ModuleElement, mouseX: Int, mouseY: Int) {
        ...
        if (moduleElement.settingsWidth < textWidth + 8) moduleElement.settingsWidth = textWidth + 8
        GlStateManager.resetColor()
        Fonts.font35.drawString(text, moduleElement.x + moduleElement.width + 6, yPos + 4, -0x1000000)
        yPos += 12
    }
    ...
    private fun drawSlider(value: Float, min: Float, max: Float, inte: Boolean, x: Int, y: Int, width: Int, mouseX: Int, mouseY: Int, color: Color?): Float {
        ...
        RenderUtils.drawRect(x.toFloat(), y.toFloat(), (x + width).toFloat(), (y + 2).toFloat(), Color(120, 120, 120).rgb)
