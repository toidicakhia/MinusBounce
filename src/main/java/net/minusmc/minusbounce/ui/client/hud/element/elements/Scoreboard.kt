...
class Scoreboard(x: Double = 5.0, y: Double = 0.0, scale: Float = 1F, side: Side = Side(Side.Horizontal.RIGHT, Side.Vertical.MIDDLE)) : Element(x, y, scale, side) {
    ...
    private val domainNameValue = TextValue("DomainName", "minusbounce.lol", {changeDomain.get()})
    private val domainFontValue = FontValue("DomainFont", Fonts.minecraftFont)

    private val domainColorRedValue = IntegerValue("Red", 0, 0, 255)
    private val domainColorGreenValue = IntegerValue("Green", 111, 0, 255)
    private val domainColorBlueValue = IntegerValue("Blue", 255, 0, 255)
    ...
    override fun drawElement(): Border? {
        ...
        val scoreCollection = scoreboard.getSortedScores(objective).filter {
            it.playerName != null && !it.playerName.startsWith("#")
        }.takeLast(15)

        val maxWidth = scoreCollection.maxOf {
            val scorePlayerTeam = scoreboard.getPlayersTeam(it.playerName)
            var name = ScorePlayerTeam.formatPlayerName(scorePlayerTeam, it.playerName)
            var stripped = StringUtils.fixString(ColorUtils.stripColor(name)!!)

            if (changeDomain.get()) {
                if (cachedDomains.contains(stripped)) {
                    name = domainNameValue.get()
                } else for (domain in domainList) 
            ...
            val width = "$name: ${EnumChatFormatting.RED}${it.scorePoints}"
            fontRenderer.getStringWidth(width)
        }.coerceAtLeast(fontRenderer.getStringWidth(objective.displayName))

        var maxHeight = scoreCollection.size * fontRenderer.FONT_HEIGHT

        val l1 = (maxWidth + 3) * if (side.horizontal == Side.Horizontal.LEFT) 1 else -1
        ...
        scoreCollection.forEachIndexed { index, score ->
            ...
            if (changed)
                fontRenderer.drawString(name, -3F, height.toFloat(), Color(domainColorRedValue.get(), domainColorGreenValue.get(), domainColorBlueValue.get()).rgb, shadowValue.get())
            else if (side.horizontal == Side.Horizontal.LEFT) 
        ...
        return if (side.horizontal == Side.Horizontal.LEFT) 
            Border(maxWidth.toFloat() + 5, -2F, -5F, maxHeight.toFloat() + fontRenderer.FONT_HEIGHT) 
        else 
            Border(-maxWidth.toFloat() - 5, -2F, 5F, maxHeight.toFloat() + fontRenderer.FONT_HEIGHT)
    }
}
