...
class Scoreboard(x: Double = 5.0, y: Double = 0.0, scale: Float = 1F, side: Side = Side(Side.Horizontal.RIGHT, Side.Vertical.MIDDLE)) : Element(x, y, scale, side) {
    ...
    override fun drawElement(): Border? {
        val antiBlind = MinusBounce.moduleManager.getModule(AntiBlind::class.java) as AntiBlind
        ...
        val hud = MinusBounce.moduleManager.getModule(HUD::class.java) as HUD
        ...
        scoreCollection.forEachIndexed { index, score ->
            ...
            if (antiSnipeMatch.get() && stripped?.let { hypickleRegex.containsMatchIn(it) } == true)
                name = ""
