...
public class MixinModelPlayer extends ModelBiped {
    ...
    public void renderHook(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale, CallbackInfo ci) {
        PlayerEdit playerEdit = MinusBounce.moduleManager.getModule(PlayerEdit.class);
        if (PlayerEdit.customModel.get() && (MinusBounce.moduleManager.getModule(PlayerEdit.class).onlyMe.get() && entityIn == Minecraft.getMinecraft().thePlayer || MinusBounce.moduleManager.getModule(PlayerEdit.class).onlyOther.get() && entityIn != Minecraft.getMinecraft().thePlayer) && MinusBounce.moduleManager.getModule(PlayerEdit.class).getState()) {
            ci.cancel();
        ...
        if (PlayerEdit.customModel.get() && (MinusBounce.moduleManager.getModule(PlayerEdit.class).onlyMe.get() && entityIn == Minecraft.getMinecraft().thePlayer || MinusBounce.moduleManager.getModule(PlayerEdit.class).onlyOther.get() && entityIn != Minecraft.getMinecraft().thePlayer) && MinusBounce.moduleManager.getModule(PlayerEdit.class).getState()) {
            if (PlayerEdit.mode.get().contains("Rabbit")) {
                ...
                if (this.isChild) {
                    ...
                    RenderUtils.INSTANCE.color(legsCustomColor); // vai dai co nx hả
                    GlStateManager.translate(0.0D, -0.15D, 0.0D);
                    this.left_leg.render(scale);
                    this.right_leg.render(scale);
